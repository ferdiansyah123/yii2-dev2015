...
class SimpelKegController extends Controller {
    ...
    public function actionUnit($unit) {
        ...
        $hitung = "SELECT count(DISTINCT(a.detail_id)) FROM serasi2015_sql.news_detail_keg a 
        INNER JOIN serasi2015_sql.news_sub_mak_tahun b on a.suboutput_id=b.suboutput_id
        INNER JOIN serasi2015_sql.news_nas_suboutput c on a.suboutput_id=c.suboutput_id 
        LEFT JOIN fix_simpel.simpel_keg g on g.detail_id = a.detail_id 
        LEFT JOIN pegawai.daf_unit d on c.unit_id=d.unit_parent_id where a.jenis_detail_id in (3,4,5) and (b.kode_mak= 524114 or b.kode_mak= 524113 or b.kode_mak= 524111 or b.kode_mak= 524119 ) and g.detail_id IS NULL and d.unit_parent_id='" . $unit . "'";
        $count = Yii::$app->db->createCommand($hitung)->queryScalar();
        $sql = "SELECT  a.detail_id, a.jenis_detail_id, a.nama_detail, a.renc_tgl_selesai, a.renc_tgl_mulai,  b.nas_prog_id, b.nas_keg_id, b.kdoutput, b.kdsoutput, b.kdkmpnen, b.kdskmpnen, b.kode_mak, d.unit_id 
        FROM serasi2015_sql.news_detail_keg as a 
         ...
         LEFT JOIN fix_simpel.simpel_keg g on a.detail_id = g.detail_id
        where " . $def;
        // echo $sql;
        // die();
    ...
    public function actionUpdate($id) {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            return $this->redirect(['/simpel-keg/tabdinas','unit'=>HelperUnit::ParentUnit($model->unit_id)]);
        } else {
    ...
    public function actionTabdinas($unit) {
        //data gridview Arsip
        $sql = "SELECT * FROM simpel_keg a INNER JOIN pegawai.daf_unit b on a.unit_id=b.unit_id WHERE status=1 and b.unit_parent_id='".$unit."' order by a.tgl_mulai desc ";
        $sqlcount = "SELECT count(a.id_kegiatan) FROM simpel_keg a INNER JOIN pegawai.daf_unit b on a.unit_id=b.unit_id WHERE status=1 and b.unit_parent_id='".$unit."' ";
    ...
    public function actionTabcetak($unit) {
        //data gridview Arsip
        $sql = "SELECT  * FROM simpel_keg a INNER JOIN pegawai.daf_unit b on a.unit_id=b.unit_id WHERE status=2 and b.unit_parent_id='".$unit."' order by a.tgl_mulai desc";
    ...
    public function actionTabuang($unit) {
        ...
        $sql = "SELECT  * FROM simpel_keg a INNER JOIN pegawai.daf_unit b on a.unit_id=b.unit_id WHERE status=3 and b.unit_parent_id='".$unit."' order by a.tgl_mulai desc";
        $count = Yii::$app->db->createCommand($sql)->queryScalar();
    ...
    public function actionTabarsip($unit) {
        //data gridview Arsip
        $sql = "SELECT  * FROM simpel_keg a INNER JOIN pegawai.daf_unit b on a.unit_id=b.unit_id WHERE a.status=4 and b.unit_parent_id='".$unit."' order by a.tgl_mulai desc";
        $count = Yii::$app->db->createCommand($sql)->queryScalar();
        // echo $sql;
        // die();
    ...
    public function actionSearchSerasi($search) {
        ...
        INNER JOIN serasi2015_sql.news_sub_mak_tahun as c ON b.sub_mak_id=c.sub_mak_id
    ...
    public function actionTambahluar($id) {
        ...
        if ($_POST) {
            ...
            $model->tgl_kembali = $_POST['SimpelKeg']['tgl_selesai'];
            $model->uang_makan = $_POST['SimpelPersonil']['uang_makan'];
            $model->angkutan = $_POST['SimpelPersonil']['angkutan'];if ($model->save()) {
                $data = count($_POST['rows']);
                for ($i = 1; $i <= $data; $i++) {
                    $model3 = new SimpelRincianBiaya();
                    $model3->id_kegiatan = $model2->id_kegiatan;
    ...
    public function actionIsi($id) {
        $model = $this->findModel($id);
        $model3 = $this->findUnit($model->unit_id);
        ...
        if (isset($_POST['TglTerima'])) {
            ...
            return $this->redirect(['tabuang','unit'=>$model3->unit_parent_id]);
        } else {
    ...
    public function actionDpr($id,$data) {
        ...
        $sql = "SELECT b.* FROM simpel_rincian_biaya  a
        INNER JOIN simpel_personil b on a.personil_id=b.id_personil
        where a.bukti_kwitansi in (2) and a.id_kegiatan=".$id." group by personil_id";
       ...
       //$model2 = \backend\models\SimpelPersonil::find()->where('id_kegiatan=' . $id)->all();
        ...
        $mpdf->Output('Pengeluaran RIll', I);
            ...
            return $this->redirect(['/site/kosong','data'=>$data]);
        }
    }
    ...
    public function actionArsip($unit) {
        ...
        Yii::$app->db->createCommand()->update('simpel_keg', ['status' => 4, 'status_edit' => 0], ['id_kegiatan' => $unit])->execute();
        return $this->redirect(['/simpel-keg/tabarsip','unit'=>$model2->unit_parent_id]);
    }
